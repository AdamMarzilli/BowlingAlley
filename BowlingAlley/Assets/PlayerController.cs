...
public class PlayerController : MonoBehaviour
{
   ...
   // [SerializeField] private Transform playerCameraTransform;
    ...
    [SerializeField] private Transform objectDragTransform;
    private ObjectDraggble objectDraggble;

    [Header("Dialogue")]
    [SerializeField] private DialougeUI dialogueUI;
    [SerializeField] public DialougeUI DialougeUI => dialogueUI;
    ...
    void Update()
    {
        ...
        {
            if(objectDraggble == null) { 
            if(Physics.Raycast(playerCamera.position, playerCamera.forward, out RaycastHit raycastHit, pickUpDistance, pickupLayerMask))
            {
                if(raycastHit.transform.TryGetComponent(out objectDraggble))
                {
                    objectDraggble.Grab(objectDragTransform); 
                    Debug.Log(objectDraggble);
                }
            }
            }
            else
